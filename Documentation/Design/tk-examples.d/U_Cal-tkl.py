...
class MCalendar(Frame):
     ...
     def initUI(self):
          ...
          ct = Label(self,text=hdr.get("name")+"'s "+hdr.get("page"), \
                     justify=CENTER).grid(row=0, columnspan=8, sticky=W+E)
          ...
          for x in range(7):
               dow = Label(self, text=days[pref.get("startDay")+x], font='serif, 10', bg="white",\
                     fg="black", height=1, width=17, borderwidth=3, relief="raised").grid(row=1, column=x+1)
          ...
          cal1 = Label(self, text=cal.get("calAt"), background=pref["calAclr"], \
                       width=17, relief="raised").grid(row=35, column=1)
